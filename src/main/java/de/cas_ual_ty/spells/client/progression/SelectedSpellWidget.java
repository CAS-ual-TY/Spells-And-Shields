...
public class SelectedSpellWidget extends GuiComponent
{
    ...
    public void drawHover(PoseStack poseStack, float deltaTick)
    {
        if(active)
        {
            ...
            blit(poseStack, x, y, 0, titleIcon * FRAME_HEIGHT, w1, FRAME_HEIGHT);
            blit(poseStack, x + w1, y, BAR_WIDTH - w2, titleIcon * FRAME_HEIGHT, w2, FRAME_HEIGHT);
            
            blit(poseStack, this.x + TITLE_PADDING_LEFT, this.y, frameIcon * FRAME_WIDTH, 128 + (spellStatus.isAvailable() ? 0 : 1) * FRAME_HEIGHT, FRAME_WIDTH, FRAME_HEIGHT);

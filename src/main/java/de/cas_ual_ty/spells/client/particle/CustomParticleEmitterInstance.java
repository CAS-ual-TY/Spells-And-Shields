 ...
 * <p>
 * {@link #spawnPosition} is this emitter's OWN construction-time capture, used only as the seed/fallback
 * {@link #spawnBatch()} falls back to when {@link #attachedTo} is null (only possible when neither attachment
 * axis is RELATIVE - see {@link #needsAttachedEntity()} - so every batch would sample the same fixed anchor
 * regardless). The anchor actually used for the ABSOLUTE position axis at render time is each particle's OWN
 * {@link CustomParticleInstance#spawnPosition}, captured fresh per BATCH by {@link #spawnBatch()} - not shared
 ...
 * batch's spawn point. Rotation has no equivalent per-particle spawn-time capture - RELATIVE rotation always
 * uses {@link #attachedTo}'s full CURRENT yaw/pitch, live, every frame, regardless of position mode (see
 * {@code CustomParticleRenderer#resolveWorldPosition}) - so mixed mode (position absolute, rotation relative)
 * reads as "stays put at its spawn position, but continuously re-orients to match wherever the entity is
 * currently facing," not a frozen-at-spawn orientation.
 * <p>
...
public class CustomParticleEmitterInstance
{
     ...
     * <p>
     * Samples {@link #attachedTo}'s CURRENT position fresh, right now, for this batch's particles' own
     * {@link CustomParticleInstance#spawnPosition} - falls back to this emitter's own {@link #spawnPosition}
     * when unattached (only possible when neither attachment axis is RELATIVE, see
     * {@link #needsAttachedEntity()}, in which case every batch would sample the same fixed anchor anyway).
     */

...
public class ManaRenderer implements LayeredDraw.Layer
{
    ...
    public void renderMana(int width, int height, GuiGraphics guiGraphics)
    {
        if(minecraft.getCameraEntity() instanceof Player player)
        {
            ...
            ManaHolder.getManaHolder(player).ifPresent(manaHolder ->
            {
                ...
                renderUnit(guiGraphics, player, left, top, rowHeight, regen, manaMax, mana, manaLast, extra, highlight);
    ...
    protected void renderUnit(GuiGraphics guiGraphics, Player player, int left, int top, int rowHeight, int regen, float manaMax, int mana, int manaLast, int extra, boolean highlight)
    {
        ...
        for(int idx = totalUnits + totalExtraUnits - 1; idx >= 0; --idx)
        {
            ...
            renderUnit(guiGraphics, UnitType.CONTAINER, x, y, v, highlight, false);
            ...
            if(renderExtra)
            {
                ...
                if(i < extra)
                {
                    ...
                    renderUnit(guiGraphics, UnitType.EXTRA, x, y, v, false, half);
                }
            }
            ...
            if(highlight && idx2 < manaLast)
            {
                ...
                renderUnit(guiGraphics, unitType, x, y, v, true, half);
            }

            if(idx2 < mana)
            {
                boolean half = idx2 + 1 == mana;
                renderUnit(guiGraphics, unitType, x, y, v, false, half);
            }
        }
    }

    private void renderUnit(GuiGraphics guiGraphics, UnitType unitType, int x, int y, int v, boolean highlight, boolean half)
    {
    ...
    public enum UnitType
    {
        ...
        {
            if(player.hasEffect(BuiltInRegisters.LEAKING_MOB_EFFECT))
            {
                return DRAINED;
            }
            else
    ...
    {
        modEventBus.addListener(ManaRenderer::registerGuiLayers);
    }
}

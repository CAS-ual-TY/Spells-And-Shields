...
public abstract class GetTargetAttributeAction<T extends Target> extends AffectSingleTypeAction<T>
{
    ...
    {
        variableAttributes.add(new VariableAttribute<>((ctx, t) -> getter.apply(t), varType, varName));
    }

    protected <C> void addVariableAttribute(BiFunction<SpellContext, T, C> getter, CtxVarType<C> varType, String varName)
    {
    ...
    protected static record VariableAttribute<T extends Target, C>(BiFunction<SpellContext, T, C> getter,
                                                                   CtxVarType<C> varType, String varName)
    {
        public void apply(SpellContext ctx, T t)
        {
            C c = getter.apply(ctx, t);
            if(c != null)
            {

...
public class MinRequirement extends Requirement
{
    ...
    public void makeDescription(List<Component> tooltip, SpellProgressionHolder spellProgressionHolder, ContainerLevelAccess access)
    {
        ...
        if(minimum == 1)
        {
            tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(getDescriptionId() + ANY_SUFFIX)));
        }
        else if(minimum == list.size())
        {
            tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(getDescriptionId() + ALL_SUFFIX)));
        }
        else
        {
            tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(getDescriptionId(), amount, minimum)));
        }

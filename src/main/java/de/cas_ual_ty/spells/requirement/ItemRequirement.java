...
public class ItemRequirement extends Requirement
{
    ...
    public ItemRequirement(IRequirementType.RequirementType type)
    {
        super(type);
    }
    
    public ItemRequirement(IRequirementType.RequirementType type, ItemStack itemStack, boolean consume)
    {
        super(type);
    ...
    public MutableComponent makeDescription(SpellProgressionHolder spellProgressionHolder, ContainerLevelAccess access)
    {
        ...
        if(itemStack.getCount() == 1)
        {
            if(consume)
            {
                return new TranslatableComponent(getDescriptionId() + CONSUMED_SUFFIX, itemStack.getHoverName());
            }
            else
            {
                return new TranslatableComponent(getDescriptionId(), itemStack.getHoverName());
            }
        }
        else
        {
            if(consume)
            {
                return new TranslatableComponent(getDescriptionId() + MULTIPLE_CONSUMED_SUFFIX, itemStack.getCount(), itemStack.getHoverName());
            }
            else
            {
                return new TranslatableComponent(getDescriptionId() + MULTIPLE_SUFFIX, itemStack.getCount(), itemStack.getHoverName());
            }
        }
    }

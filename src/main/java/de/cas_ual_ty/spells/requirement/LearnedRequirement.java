...
public class LearnedRequirement extends Requirement
{
    ...
    public void makeDescription(List<Component> tooltip, SpellProgressionHolder spellProgressionHolder, ContainerLevelAccess access)
    {
        if(spellProgressionHolder.getPlayer().level instanceof ServerLevel level)
        {
            ...
            if(tree == null)
            {
                tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(descriptionId + ERROR_TREE_SUFFIX, nodeId.treeId())));
                return;
            }
            ...
            if(node == null)
            {
                tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(descriptionId + ERROR_NODE_SUFFIX, tree.getTitle(), nodeId.nodeId())));
                return;
            }
            
            SpellInstance spell = node.getSpellInstance();
            tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(descriptionId, spell.getSpell().value().getTitle(), tree.getTitle())));
        }
    }

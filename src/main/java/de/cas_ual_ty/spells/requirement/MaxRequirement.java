...
public class MaxRequirement extends Requirement
{
    ...
    public void makeDescription(List<Component> tooltip, SpellProgressionHolder spellProgressionHolder, ContainerLevelAccess access)
    {
        ...
        if(maximum == 0)
        {
            if(list.size() == 1)
            {
                tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(getDescriptionId() + NOT_SUFFIX)));
            }
            else
            {
                tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(getDescriptionId() + NONE_SUFFIX)));
            }
        }
        else
        {
            tooltip.add(formatComponent(spellProgressionHolder, access, SpellsDowngrade.translatable(getDescriptionId(), amount, maximum)));
        }
        
        List<Component> subTooltip = new LinkedList<>();
        list.forEach(r -> r.makeDescription(subTooltip, spellProgressionHolder, access));
        subTooltip.stream().map(c -> SpellsDowngrade.literal("  ").append(c)).forEach(tooltip::add);
    }

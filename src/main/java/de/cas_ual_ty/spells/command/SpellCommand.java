...
public class SpellCommand
{
        ...
        dispatcher.register(Commands.literal("spells").requires(css -> css.hasPermission(2))
                ...
                .then(Commands.literal("cast")
                        .then(Commands.argument(ARG_TARGETS, EntityArgument.player())
                                .then(Commands.literal("direct").then(Commands.argument(ARG_SPELL, SpellArgument.spell()).executes(SpellCommand::spellsCastDirect)))
                                .then(Commands.argument(ARG_SPELL_TREE, SpellTreeArgument.spellTree()).then(Commands.argument(ARG_NODE_ID, IntegerArgumentType.integer()).executes(SpellCommand::spellsCast)))
                        )
                )
        );
    }
    ...
    private static int spellsCastDirect(CommandContext<CommandSourceStack> context) throws CommandSyntaxException
    {
        ...
        context.getSource().sendSuccess(SpellsDowngrade.translatable(SPELLS_CAST_DIRECT, players.iterator().next().getDisplayName(), spell.getTitle()), true);
    ...
    private static int spellsCast(CommandContext<CommandSourceStack> context) throws CommandSyntaxException
    {
        ...
        Spell spell = spellNode.getSpellDirect();
        context.getSource().sendSuccess(SpellsDowngrade.translatable(SPELLS_CAST, players.iterator().next().getDisplayName(), spell.getTitle(), spellTree.getTitle()), true);

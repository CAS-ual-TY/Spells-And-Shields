...
public class SpellDataHolder implements ISpellDataHolder
{
    ...
    @Override
    public <D extends SpellData> List<D> getAllOfType(ISpellDataType type)
    {
        List<D> list = new LinkedList<>();
        forEachOfType(type, data -> list.add((D) data));
        return list;
    }
    
    @Override
    public <D extends SpellData> void forEachOfType(ISpellDataType type, Consumer<D> consumer)
    {
